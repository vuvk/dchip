...
alias cpPolylineConvexDecomposition = cpPolylineConvexDecomposition_BETA;


static int Next(int i, int count)
{
	return (i+1)%count;
}
...
enum DEFAULT_POLYLINE_CAPACITY = 16;

static ulong cpPolylineSizeForCapacity(int capacity)
{
	return cpPolyline.sizeof + capacity*cpVect.sizeof;
}
...
static cpPolyline* cpPolylineMake(int capacity)
{
	...
	cpPolyline* line = cast(cpPolyline*)cpcalloc(1, cpPolylineSizeForCapacity(capacity));
	line.count = 0;
...
static cpPolyline* cpPolylineShrink(cpPolyline* line)
{
	...
	return cast(cpPolyline*)cprealloc(line, cpPolylineSizeForCapacity(line.count));
}
...
static cpPolyline* cpPolylineGrow(cpPolyline* line, int count)
{
	...
	{
		line.capacity = capacity;
		line = cast(cpPolyline*)cprealloc(line, cpPolylineSizeForCapacity(capacity));
	}
...
static cpPolyline* cpPolylineEnqueue(cpPolyline* line, cpVect v)
{
	...
	int count = line.count;
	line = cpPolylineGrow(line, 1);
	memmove(cast(cpVect*)line.verts + 1, cast(cpVect*)line.verts, count*cpVect.sizeof);
	line.verts[0] = v;
...
static cpBool cpPolylineIsShort(cpVect* points, int count, int start, int end, cpFloat min)
{
	cpFloat length = 0.0f;
...
static cpFloat Sharpness(cpVect a, cpVect b, cpVect c)
{
...
static cpPolyline* DouglasPeucker(cpVect* verts, cpPolyline* reduced,
...
cpPolyline* cpPolylineSimplifyCurves(cpPolyline* line, cpFloat tol)
{
	...
	{
		int start, end;
		cpLoopIndexes(cast(cpVect*)line.verts, line.count - 1, &start, &end);
    
		reduced = cpPolylinePush(reduced, line.verts[start]);
		reduced = DouglasPeucker(cast(cpVect*)line.verts, reduced, line.count - 1, start, end, min, tol);
		reduced = cpPolylinePush(reduced, line.verts[end]);
		reduced = DouglasPeucker(cast(cpVect*)line.verts, reduced, line.count - 1, end, start, min, tol);
		reduced = cpPolylinePush(reduced, line.verts[start]);
	} 
	else 
	{
		reduced = cpPolylinePush(reduced, line.verts[0]);
		reduced = DouglasPeucker(cast(cpVect*)line.verts, reduced, line.count, 0, line.count - 1, min, tol);
...
{
	return cast(cpPolylineSet*)cpcalloc(1, cpPolylineSet.sizeof);
}
...
cpPolylineSet* cpPolylineSetInit(cpPolylineSet* set)
{
	set.count = 0;
	set.capacity = 8;
	set.lines = cast(cpPolyline**)cpcalloc(set.capacity, cpPolylineSet.sizeof);
...
static void cpPolylineSetPush(cpPolylineSet* set, cpPolyline* line)
{
	...
	{
		set.capacity *= 2;
		set.lines = cast(cpPolyline**)cprealloc(set.lines, set.capacity*cpPolyline.sizeof);
	}
...
static void cpPolylineSetJoin(cpPolylineSet* set, int before, int after)
{
	...
	memmove(cast(cpVect*)lbefore.verts + count, cast(cpVect*)lafter.verts, lafter.count*cpVect.sizeof);
...
cpPolyline* cpPolylineToConvexHull(cpPolyline* line, cpFloat tol)
{
	...
	hull.count = cpConvexHull(line.count, cast(cpVect*)line.verts, cast(cpVect*)hull.verts, null, tol);
...
static cpFloat FindSteiner(int count, cpVect* verts, Notch notch)
{
...
static Notch DeepestNotch(int count, cpVect* verts, int hullCount, cpVect* hullVerts, int first, cpFloat tol)
{
	Notch notch = {};
...
static void ApproximateConcaveDecomposition(cpVect* verts, int count, cpFloat tol, cpPolylineSet* set)
{
	int first;
	...
	if(hullCount != count)
	{
		Notch notch = DeepestNotch(count, verts, hullCount, hullVerts, first, tol);
		...
		if(notch.d > tol)
		{
			...
			if(steiner_it >= 0.0)
			{
				int steiner_i = cast(int)steiner_it;
				...
				cpVect* scratch = cast(cpVect*)alloca((IMAX(sub1_count, sub2_count) + 1)*cpVect.sizeof);
	...
	memcpy(cast(cpVect*)hull.verts, hullVerts, hullCount*cpVect.sizeof);
...
cpPolylineSet* cpPolylineConvexDecomposition_BETA(cpPolyline* line, cpFloat tol)
{
	...
	cpAssertSoft(cpAreaForPoly(line.count, cast(cpVect*)line.verts, 0.0) >= 0.0, "Winding is backwards. (Are you passing a hole?)");
	
	cpPolylineSet* set = cpPolylineSetNew();
	ApproximateConcaveDecomposition(cast(cpVect*)line.verts, line.count - 1, tol, set);
